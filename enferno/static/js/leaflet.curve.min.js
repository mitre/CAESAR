(L.Curve = L.Path.extend({
  ...
  _computeBounds: function () {
    for (
      var t, n, i, e = new L.LatLngBounds(), a = 0;
      a < this._coords.length;
      a++
    )
      if ("string" == typeof (i = this._coords[a]) || i instanceof String)
        ...
        o = new L.latLng(i[0], i[1]);
        i = this._coords[++a];
        (r = new L.latLng(i[0], i[1])), (s = t);
        if (t.controlPoint2) {
          var h = t.lat - t.controlPoint2.lat,
            c = t.lng - t.controlPoint2.lng;
        ...
        i = this._coords[++a];
        r = new L.latLng(i[0], i[1]);
        e.extend(l), e.extend(r), (r.controlPoint = l), (t = r);
      } else if ("T" == n) {
        (r = new L.latLng(i[0], i[1])), (l = t);
        if (t.controlPoint) {
          (h = t.lat - t.controlPoint.lat), (c = t.lng - t.controlPoint.lng);
          l = new L.latLng(t.lat + h, t.lng + c);
        }
        e.extend(l), e.extend(r), (r.controlPoint = l), (t = r);
      } else e.extend(i), (t = new L.latLng(i[0], i[1]));
    return e;
  },
  getCenter: function () {
    ...
    for (var a = 0; a < this._coords.length; a++)
      if ("string" == typeof (t = this._coords[a]) || t instanceof String)
        ...
        switch (t.length) {
          ...
          case 1:
            "H" == i
              ? ((e = this._map.latLngToLayerPoint([n[0], t[0]])),
                (n = [n[0], t[0]]))
              : ((e = this._map.latLngToLayerPoint([t[0], n[1]])),
                (n = [t[0], n[1]]));
        }
        this._points.push(e);
      }
  ...
  _curvePointsToPath: function (t) {
    for (var n, i, e = "", a = 0; a < t.length; a++)
      if ("string" == typeof (n = t[a]) || n instanceof String) e += i = n;
      else
        switch (i) {
          case "H":
            e += n.x + " ";
  ...
  beforeAdd: function (t) {
    L.Path.prototype.beforeAdd.call(this, t),
  ...
  onAdd: function (t) {
    if (
      ...
      L.Path.prototype.onAdd.call(this, t),
      this._usingCanvas)
    )
      this.options.animate && "object" == typeof TWEEN
        ? (this._normalizeCanvasAnimationOptions(),
          ...
          (this._tween = new TWEEN.Tween(this._tweenedObject)
            ...
            .onComplete(
              ((i = this),
              function () {
                i._canvasAnimating = !1;
              }),
            )
            .start()),
          (this._canvasAnimating = !0),
          this._animateCanvas())
        : (this._canvasAnimating = !1);
    else if (this.options.animate && this._path.animate) {
      var n = Math.min(this._svgSetDashArray(), 1e3);
      (this._path.pathLength.baseVal = n),
        this._path.animate(
          [{ strokeDashoffset: n }, { strokeDashoffset: 0 }],
  ...
  _containsPoint: function (t) {
    return (
      ...
      this._bounds.contains(this._map.layerPointToLatLng(t))
    );
  },
  _normalizeCanvasAnimationOptions: function () {
    var t = { delay: 0, duration: 0, iterations: 1 };
    "number" == typeof this.options.animate
      ? (t.duration = this.options.animate)
      : (this.options.animate.duration &&
          (t.duration = this.options.animate.duration),
        this.options.animate.delay && (t.delay = this.options.animate.delay),
        this.options.animate.iterations &&
          (t.iterations = this.options.animate.iterations)),
      (this.options.animate = t);
  },
  _updateCurveCanvas: function () {
    var t = this._curvePointsToPath(this._points);
    this._pathSvgElement.setAttribute("d", t),
      this.options.animate &&
        "object" == typeof TWEEN &&
        this._canvasSetDashArray &&
        ((this.options.dashArray = this._pathSvgElement.getTotalLength() + ""),
        this._renderer._updateDashArray(this)),
      this._curveFillStroke(new Path2D(t), this._renderer._ctx);
  },
  ...
  _curveFillStroke: function (t, n) {
    ...
    var i = this.options;
    i.fill &&
      ((n.globalAlpha = i.fillOpacity),
      (n.fillStyle = i.fillColor || i.color),
      n.fill(t, i.fillRule || "evenodd")),
      i.stroke &&
        0 !== i.weight &&
        (n.setLineDash &&
          n.setLineDash((this.options && this.options._dashArray) || []),
        (n.globalAlpha = i.opacity),
        (n.lineWidth = i.weight),
        ...
        switch (i) {
          ...
          case "C":
            (s = n),
              (o = this._points[++c]),
              (r = this._points[++c]),
              (h = h.concat(this._cubicTrace(t, a, s, o, r))),
              (a = r);
            break;
          case "S":
            (s = this._reflectPoint(o, a)),
              (o = n),
              (r = this._points[++c]),
              (h = h.concat(this._cubicTrace(t, a, s, o, r))),
              (a = r);
          ...
          case "Q":
            (s = n),
              (o = this._points[++c]),
              (h = h.concat(this._quadraticTrace(t, a, s, o))),
              (a = o);
            break;
          case "T":
            (s = this._reflectPoint(s, a)),
              (o = n),
              (h = h.concat(this._quadraticTrace(t, a, s, o))),
              (a = o);
        }
  ...
  _linearTrace: function (t, n, i) {
    return t.map((t) => {
      var e = this._singleLinearTrace(t, n.x, i.x),
        a = this._singleLinearTrace(t, n.y, i.y);
      return this._map.layerPointToLatLng([e, a]);
    });
  },
  _quadraticTrace: function (t, n, i, e) {
    return t.map((t) => {
      var a = this._singleQuadraticTrace(t, n.x, i.x, e.x),
        s = this._singleQuadraticTrace(t, n.y, i.y, e.y);
      return this._map.layerPointToLatLng([a, s]);
    });
  },
  _cubicTrace: function (t, n, i, e, a) {
    return t.map((t) => {
      var s = this._singleCubicTrace(t, n.x, i.x, e.x, a.x),
        o = this._singleCubicTrace(t, n.y, i.y, e.y, a.y);
