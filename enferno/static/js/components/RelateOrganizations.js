Vue.component("relate-organizations", {
  ...
  methods: {
    viewOrganization(a) {
      axios
        .get(`/admin/api/organization/${a.id}`)
        .then((response) => {
    ...
    search(q = {}) {
      ...
      axios
        .post(
          `/admin/api/organizations/?page=${this.page}&per_page=${this.perPage}&mode=2`,
          { q: [this.q] },
        )
        .then((response) => {
      ...
      <v-sheet>
        ...
        <v-container class="fluid fill-height">
          <v-row>
            <v-col cols="12" md="4">
              <v-card outlined>
                <organization-search-box @search="reSearch" :i18n="$root.translations" v-model="q"
                                  :show-op="false"></organization-search-box>
            ...
            <v-col cols="12" md="8">
              ...
              <v-card :loading="loading">
                ...
                <v-card class="pa-2" tile color="grey lighten-4">
                  ...
                  <organization-result :i18n="i18n" v-for="(item, i) in results" :key="i" :organization="item" :show-hide="true">
        ...
        <v-dialog v-model="showOrganization" max-width="550">
          <v-sheet>
            <div class="d-flex justify-end">
              <v-btn @click="showOrganization=false" small text fab right="10">
                <v-icon>mdi-close</v-icon>
              </v-btn>
            </div>
